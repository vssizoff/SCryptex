function hash(data, salt = 10) {return bcrypt.hashSync(data, salt)}

let compareHash = compareSync;
...
async function RSAPost(data, url, keyUrl, options = defaultCfg) {
    options = configValidator(defaultCfg, options);
    let obj = await getRequestSync(keyUrl, {
        query: options.getKeyQuery,
        outputType: options.getKeyOutputType,
        headers: options.getKeyHeaders
    });
    let key = options.getKey(...obj);
    return postRequestSync(url, RSAEncrypt(data, key), options);
}